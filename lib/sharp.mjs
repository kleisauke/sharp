...
function loadNativeModule(filename) {
  const targetModule = { exports: {} };
  dlopen(targetModule, filename, constants.dlopen.RTLD_LAZY | (constants.dlopen.RTLD_DEEPBIND || 0));
...
const errors = [];
try {
  sharp = loadNativeModule(require.resolve(`../src/build/Release/sharp-${runtimePlatform}-${version}.node`));
} catch (err) {
  errors.push(err);
}
if (!sharp) {
  ...
  try {
    switch (runtimePlatform) {
      ...
      case "darwin-x64":
        ...
        break;
      case "linux-arm":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-arm/sharp.node"));
        break;
      case "linux-arm64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-arm64/sharp.node"));
        break;
      case "linux-ppc64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-ppc64/sharp.node"));
        break;
      case "linux-riscv64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-riscv64/sharp.node"));
        break;
      case "linux-s390x":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-s390x/sharp.node"));
        break;
      case "linux-x64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linux-x64/sharp.node"));
        break;
      case "linuxmusl-arm64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linuxmusl-arm64/sharp.node"));
        break;
      case "linuxmusl-x64":
        sharp = loadNativeModule(require.resolve("@img/sharp-linuxmusl-x64/sharp.node"));
        break;
      case "win32-arm64":
